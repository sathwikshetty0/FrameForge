...
describe('Detection Engine - analyze', () => {
  describe('MISSING_EXIF signal', () => {
    it('should not trigger when 3 or more core fields are present', async () => {
      const metadata = createGenuineMetadata();
      const signals = await analyze(metadata, 5000000);
      const missingExif = signals.filter((s) => s.type === 'MISSING_EXIF');
      expect(missingExif).toHaveLength(0);
    });

    it('should trigger when fewer than 3 core fields are present', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should have severity 1.0 when 0 core fields are present', async () => {
      const metadata = createGenuineMetadata();
      metadata.cameraMake = absent();
      metadata.cameraModel = absent();
      metadata.dateTimeOriginal = absent();
    ...
    it('should have severity 2/3 when only 1 core field is present', async () => {
      const metadata = createGenuineMetadata();
      metadata.cameraMake = absent();
      metadata.cameraModel = absent();
      metadata.dateTimeOriginal = absent();
  ...
  describe('SOFTWARE_FINGERPRINT signal', () => {
    it('should not trigger when software is absent', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should not trigger when software does not match any keyword', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger for "DALL-E" (case-insensitive)', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger for "stable diffusion" in mixed case', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger for "Photoshop"', async () => {
      const metadata = createGenuineMetadata();
  ...
  describe('TIMESTAMP_INCONSISTENCY signal', () => {
    it('should not trigger when timestamps are within 24 hours', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger when timestamps differ by more than 24 hours', async () => {
      const metadata = createGenuineMetadata();
      metadata.dateTimeOriginal = present(new Date('2024-01-15T10:00:00'));
    ...
    it('should cap severity at 1.0 for very large differences', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger with severity 0.5 when dateTimeOriginal is absent', async () => {
      const metadata = createGenuineMetadata();
      metadata.dateTimeOriginal = absent();
      const signals = await analyze(metadata, 5000000);
      const ts = signals.filter((s) => s.type === 'TIMESTAMP_INCONSISTENCY');
    ...
    it('should trigger with severity 0.5 when modifyDate is absent', async () => {
      const metadata = createGenuineMetadata();
  ...
  describe('FILE_SIZE_ANOMALY signal', () => {
    it('should not trigger when ratio is between 0.2 and 5.0', async () => {
      const metadata = createGenuineMetadata();
      ...
      const signals = await analyze(metadata, 10000000);
      const fs = signals.filter((s) => s.type === 'FILE_SIZE_ANOMALY');
      expect(fs).toHaveLength(0);
    });

    it('should trigger when ratio < 0.2 (very small file)', async () => {
      const metadata = createGenuineMetadata();
      metadata.imageWidth = present(4000);
      metadata.imageHeight = present(3000);
      // Expected = 36,000,000, ratio = 100000/36000000 ≈ 0.003
      const signals = await analyze(metadata, 100000);
      const fs = signals.filter((s) => s.type === 'FILE_SIZE_ANOMALY');
    ...
    it('should trigger when ratio > 5.0 (very large file)', async () => {
      const metadata = createGenuineMetadata();
      metadata.imageWidth = present(4000);
      metadata.imageHeight = present(3000);
      // Expected = 36,000,000, ratio = 200,000,000/36,000,000 ≈ 5.56
      const signals = await analyze(metadata, 200000000);
      const fs = signals.filter((s) => s.type === 'FILE_SIZE_ANOMALY');
      expect(fs).toHaveLength(1);
    });

    it('should not trigger when image dimensions are absent', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should compute severity as min(1.0, abs(ratio - 1) / 4)', async () => {
      const metadata = createGenuineMetadata();
  ...
  describe('COLOR_PROFILE_ABNORMALITY signal', () => {
    it('should not trigger when color profile is present and bit depth is 8', async () => {
      const metadata = createGenuineMetadata();
      const signals = await analyze(metadata, 5000000);
    ...
    it('should not trigger when color profile is present and bit depth is 16', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger with severity 0.5 when only color profile is absent', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger with severity 0.5 when only bit depth is abnormal', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger with severity 1.0 when both conditions met', async () => {
      const metadata = createGenuineMetadata();
      metadata.colorProfile = absent();
      metadata.bitDepth = present(32);
      const signals = await analyze(metadata, 5000000);
  ...
  describe('MISSING_GPS signal', () => {
    it('should not trigger when both GPS coordinates are present', async () => {
      const metadata = createGenuineMetadata();
      const signals = await analyze(metadata, 5000000);
    ...
    it('should trigger when GPS latitude is absent', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger when GPS longitude is absent', async () => {
      const metadata = createGenuineMetadata();
    ...
    it('should trigger when both GPS coordinates are absent', async () => {
      const metadata = createGenuineMetadata();
      metadata.gpsLatitude = absent();
      metadata.gpsLongitude = absent();
      const signals = await analyze(metadata, 5000000);
  ...
  describe('Signal structure', () => {
    it('all signals have valid type, severity in [0,1], and non-empty triggerField', async () => {
      const metadata = createGenuineMetadata();
      ...
      const signals = await analyze(metadata, 100); // tiny file for anomaly
